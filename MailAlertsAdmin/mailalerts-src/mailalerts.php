...
class MailAlerts extends Module
{
	...
	public function install()
	{
		...
		Configuration::updateValue('MA_CUSTOMER_MAILS', Configuration::get('PS_SHOP_EMAIL'));
	...
	public function uninstall()
	{
		...
		Configuration::deleteByName('MA_CUSTOMER_MAILS');
	...
	{
		$this->_customer_mails = Configuration::get('MA_CUSTOMER_MAILS');
...
//		$order_date_text = Tools::displayDate($order->date_add, intval($id_lang));
	...
	public function hookProductOutOfStock($params)
	{
		...
		else
		{
			...
			if ($this->customerHasNotification($id_customer, $id_product, $id_product_attribute))
    			$smarty->assign('customerHasNotification', 1);
		}
	...
	public function hookUpdateQuantity($params)
	{
		...
		if ($qty <= intval(Configuration::get('mA_last_qties')) AND !(!$this->_merchant_oos OR empty($this->_merchant_mails)) AND Configuration::get('PS_STOCK_MANAGEMENT'))
		{
			...
			if (file_exists(dirname(__FILE__).'/mails/'.$iso.'/productoutofstock.txt') AND file_exists(dirname(__FILE__).'/mails/'.$iso.'/productoutofstock.html'))
				Mail::Send($id_lang, 'productoutofstock', $this->l('Product out of stock'), $templateVars, explode(self::__MA_MAIL_DELIMITOR__, $this->_merchant_mails), NULL, strval(Configuration::get('PS_SHOP_EMAIL')), strval(Configuration::get('PS_SHOP_NAME')), NULL, NULL, dirname(__FILE__).'/mails/');
		}
	...
	public function sendCustomerAlert($id_product, $id_product_attribute)
	{
		...
		$product =  new Product(intval($id_product), false, $id_lang);
		$templateVars = array(
		...
		foreach ($customers as $cust)
		{
			...
			if (file_exists(dirname(__FILE__).'/mails/'.$iso.'/customer_qty.txt') AND file_exists(dirname(__FILE__).'/mails/'.$iso.'/customer_qty.html'))
				Mail::Send($id_lang, 'customer_qty', $this->l('Product available'), $templateVars, strval($customer_email), NULL, strval(Configuration::get('PS_SHOP_EMAIL')), strval(Configuration::get('PS_SHOP_NAME')), NULL, NULL, dirname(__FILE__).'/mails/');
			if ($customer_id)
				$customer_email = 0;
			...
			<fieldset class="width3"><legend><img src="'.$this->_path.'logo.gif" />'.$this->l('Customer notification').'</legend>
				...
				<div class="margin-form">
					<div style="float:left; margin-right:10px;">
						<textarea name="mA_customer_mails" rows="10" cols="30">'.Tools::getValue('mA_customer_mails', str_replace(self::__MA_MAIL_DELIMITOR__, "\n", $this->_customer_mails)).'</textarea>
					</div>
					<div style="float:left;">
						'.$this->l('One email address per line').'<br />
						'.$this->l('e.g.,').' bob@example.com<br />
						'.$this->l('Leave blank to send no email.').'
					</div>
				</div>
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('submitMACustomer'))
		{
			...
			else
            {
                ...
                $emails = strval(Tools::getValue('mA_customer_mails'));
                $emails = explode("\n", $emails);
                ...
                if (!Configuration::updateValue('MA_CUSTOMER_MAILS', strval($emails)))
                    $this->_html .= '<div class="alert error">'.$this->l('Cannot update settings').'</div>';
            }
		}
    ...
    public function sendNewAlertEmails($params)
    {
		if (empty($this->_customer_mails))
			return;
        ...
        $id_lang = (( ! isset($cookie)) OR ( ! is_object($cookie)))
                 ? intval(Configuration::get('PS_LANG_DEFAULT'))
                 : intval($cookie->id_lang);
        $configuration = Configuration::getMultiple(array('PS_SHOP_EMAIL', 'PS_MAIL_METHOD', 'PS_MAIL_SERVER', 'PS_MAIL_USER', 'PS_MAIL_PASSWD', 'PS_SHOP_NAME', 'PS_SHOP_LONG_NAME'));
        // Filling-in vars for email
        $template = 'new_alert';
        $subject = $this->l('New alert');
        $templateVars = array(
            '{shop_name}' => $configuration['PS_SHOP_NAME'],
            '{shop_long_name}' => $configuration['PS_SHOP_LONG_NAME'],
            '{shop_email}' => $configuration['PS_SHOP_EMAIL'],
            '{id_customer}' => $params['id_customer'],
            '{customer_email}' => $params['customer_email'],
            '{id_product}' => $params['id_product'],
            '{name_product}' => $params['name_product'],
            '{id_product_attribute}' => $params['id_product_attribute'],
            '{already_registered_alert}' => $params['already_registered_alert'],
        );
        $iso = Language::getIsoById(intval($id_lang));
        if (file_exists(dirname(__FILE__).'/mails/'.$iso.'/'.$template.'.txt') AND file_exists(dirname(__FILE__).'/mails/'.$iso.'/'.$template.'.html'))
            Mail::Send($id_lang, $template, $subject, $templateVars, explode(self::__MA_MAIL_DELIMITOR__, $this->_customer_mails), NULL, $configuration['PS_SHOP_EMAIL'], $configuration['PS_SHOP_NAME'], NULL, NULL, dirname(__FILE__).'/mails/');
    }
